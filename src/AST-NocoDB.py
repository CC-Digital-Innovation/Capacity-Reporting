...
app = FastAPI(
    ...
    description= "Downloads EQ-AST data from NocoDB as CSV"
)

config = configparser.ConfigParser()
config.read('config-NocoDB.ini')
URL = config.get('NocoDB', 'url')
URL_MULTI = config.get('NocoDB', 'url_multi')
...
async def get_Multiple_Storage_Capacity_Reportings(start_date: str, 
    ...
    r = requests.get(URL_MULTI, headers=header)
    dictionary = json.loads(r.text)
